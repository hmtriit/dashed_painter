...
void main() {
  group('DashedPainter logic', () {
    ...
    test('calculates partLength correctly with dots', () {
      const painter = DashedPainter(step: 4.0, span: 2.0, pointCount: 2, pointWidth: 1.0);
      const expected = 4.0 + 2.0 * 3 + 1.0 * 2; // step + span*(dots+1) + dotLen*dots = 12
      final partLength = painter.step +
    ...
    test('uses strokeWidth when pointWidth is null', () {
      const painter = DashedPainter(pointCount: 2, step: 4, span: 2, pointWidth: null);
